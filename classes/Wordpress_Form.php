...
Class Wordpress_Form {
	...
	function set_forminfo_tag( $name, $classes = false ) {
		...
		if ( $classes ) {
			$this->forminfo_tag_classes = is_array( $classes ) ? $classes : array( $classes );
		}
	}
	...
	function isEditorFor( $setting ) {
		$this->isEditorFor = $setting;
	}
	...
	function add_fieldset( $name, $legend = false, $fieldsetinfo = false ) {
		if ( $legend ) {
			$this->fieldsets[ $name ]['legend'] = $legend;
		}
		if ( $fieldsetinfo ) {
			$this->fieldsets[ $name ]['fieldsetinfo'] = $fieldsetinfo;
		}
	}
	...
	function add_textarea( $fieldset, $name, $label, $classes = false, $forminfo = false, $value = false ) {
		...
		if ( ! $value ) {
			...
			switch ( $this->isEditorFor ) {
				case "currentUserMeta":
		...
		if ( wp_verify_nonce( $_POST['nonce'], 'wordpress_form_submit' ) ) {
			$this->meta_array[ $name ] = $_POST[ $name ];
			$value = $_POST[ $name ];
		}
	...
	function add_list_box( $fieldset, $name, $label, $options, $classes = false, $forminfo = false, $value = false ) {
		...
		if ( $forminfo ) {
			...
			$output .= $this->forminfo_tag_classes ? " class='" . implode( ' ',
					$this->forminfo_tag_classes ) . "'" : null;
	...
	function add_file_upload(
		...
		$name,
		$label,
		$attachment_id_metafield_name = null,
		$classes = false,
		$forminfo = false
	) {
		...
		switch ( $_FILES[ $name ]["error"] ) {
			case UPLOAD_ERR_INI_SIZE:
				$field_array['error'] = 'The uploaded file exceeds the upload_max_filesize directive in php.ini';
				break;
			case UPLOAD_ERR_FORM_SIZE :
				$field_array['error'] = 'The uploaded file exceeds the MAX_FILE_SIZE directive that was specified in the HTML form.';
				break;
			case UPLOAD_ERR_PARTIAL :
				$field_array['error'] = 'The uploaded file was only partially uploaded.';
				break;
			case UPLOAD_ERR_NO_TMP_DIR :
				$field_array['error'] = 'Missing a temporary folder.';
				break;
			case UPLOAD_ERR_CANT_WRITE :
				$field_array['error'] = 'Failed to write file to disk.';
				break;
			case UPLOAD_ERR_EXTENSION :
				$field_array['error'] = 'A PHP extension stopped the file upload.';
				break;
		}
		...
		if ( $_POST ) {
			$this->file_array[ $name ] = $attachment_id_metafield_name;
		}
			...
			switch ( $this->isEditorFor ) {
				...
				case "currentUserMeta":
					...
					foreach ( $this->meta_array as $key => $value ) {
						update_user_meta( get_current_user_id(), $key, esc_attr( $value ) );
					}
					break;
	...
	function form_output( $echo = true ) {
		...
		foreach ( $this->fields as $fieldset_name => $fields ) {
			if ( $this->use_fieldsets ) {
				...
				if ( isset ( $this->fieldsets[ $fieldset_name ]['legend'] ) ) {
					if ( $this->fieldsets[ $fieldset_name ]['legend'] ) {
						$output .= "\t\t<legend>" . $this->fieldsets[ $fieldset_name ]['legend'] . "</legend>\n";
					}
				}

				if ( isset ( $this->fieldsets[ $fieldset_name ]['fieldsetinfo'] ) ) {
					if ( $this->fieldsets[ $fieldset_name ]['fieldsetinfo'] ) {
						$output .= "\t\t<p class='info'>" . $this->fieldsets[ $fieldset_name ]['fieldsetinfo'] . "</p>\n";
					}
				}
			}
		...
		$output .= $this->submit_button_classes ? " class='" . implode( ' ',
				$this->submit_button_classes ) . "'" : null;
