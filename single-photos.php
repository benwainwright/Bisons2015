...
<?php while ( have_posts() ) : the_post();
    ...
    $photo_rows = array_chunk ( $photos, isset ( $_GET['cols'] ) ? $_GET['cols'] : 5 );
    ?>
    ...
    <?php endif ?>
        <header>
            <h2><a href="<?php the_permalink() ?>"><?php the_title() ?></a></h2>
	      	<ul class='pageMenu'>
	          <li><span class='fa fa-calendar fa-lg'>Album created on <a href='https://www.flickr.com/photos/bisonsrfc/'>Flickr</a> on the <?php the_date('jS \o\f F Y') ?></span></li>
	      	</ul>
        </header>
