...
<div id='custom-form'>
	<p>Please <a href='<?php echo admin_url( 'admin.php?page=add-player' ) ?>'>add any new players</a> to the database
		before recording the register to ensure their attendance is recorded also.</p>
	<table class="form-table">
		<tbody>
		<tr class='smallFormRow'>
			<th><label for="register_date">Date</label></th>
			<td>
				<input type='date' class="required" name='reg-date'
				       value='<?php echo get_post_meta( $post->ID, 'reg-date', true ) ? date( 'Y-m-d',
					       get_post_meta( $post->ID, 'reg-date', true ) ) : date( 'Y-m-d' ); ?>'/>
			...
			<td>
				...
				<select class='register_listbox' id='players_present' multiple="multiple" name="players_present[]">
					...
					foreach ( $users as $user ) : ?>
						<option <?php if ( array_search( $user->data->ID, $selected ) !== false ) {
							echo "selected='selected' "} ?>value='<?php echo $user->data->ID ?>'><?php echo $user->data->display_name ?></option>
					<?php endforeach; ?>
				</select>
		...
		<?php if ( $absent = get_post_meta( $post->ID, 'players_absent', false ) ) : ?>
			<tr>
		...
		<tr>
			<td class='formButtonCell' colspan='2'><input type="submit" name="publish" id="publish"
			                                              class="button button-primary button-large resultsButton"
			                                              value="Save" accesskey="p">
</div></td>
